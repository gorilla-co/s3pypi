...
UNEXPECTED_EVENT_RESPONSE = Response(
    {"status": "500", "statusDescription": "Unexpected LambdaEdge event"}
)


UNSUPPORTED_METHOD_RESPONSE = Response(
    {"status": "405", "statusDescription": "Method not allowed"}
)
...
def s3_user_store(distribution_id: str) -> users.S3UserStore:
    ...
    pypi_bucket_origin = next(
        filter(lambda origin: origin.get("Id") == "PyPiS3BucketOrigin", origins), {}
    )
    bucket_dn = pypi_bucket_origin.get("DomainName")
    bucket_name = bucket_dn.rsplit(".", maxsplit=4)[
        0
    ]  # <bucket_name>.s3.<region>.amazonaws.com
    return users.S3UserStore(
        bucket_name, "config/users", refresh_period=60.0, aws_session=aws_session
    )
...
def get_basic_auth_credentials(
    request: Request
) -> t.Optional[UsernamePasswordCredentials]:
    """
    ...
    try:
        ...
        log.info(
            "parsing of basic authorization header failed with an exception",
            exc_info=ex,
        )
