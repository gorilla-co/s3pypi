...
_PROJECT_ROOT: pathlib.Path = pathlib.Path(__file__).parents[len(__package__.split('.'))]
...
class Parameter:
    ...
    def to_input_json(self) -> t.Dict[str, t.Union[str, bool]]:
        result = {
            'ParameterKey': self.name,
            'UsePreviousValue': self.use_previous
        }
...
class CloudFormationService:
    ...
    def create_stack(self,
                     ...
                     waiter_delay: int = 30,
                     waiter_max_attempts: int = 120) -> t.Mapping[str, t.Any]:
        """
        ...
        stack = self.cloudformation_service.create_stack(
            ...
            Parameters=list(map(Parameter.to_input_json, _force_list(parameters, Parameter))),
            Capabilities=_force_list(capabilities, str),
            OnFailure='ROLLBACK'
        )
        _log.info(f'waiting for the creation of stack {stack_name} to complete')
        self._get_waiter('stack_create_complete').wait(StackName=stack.stack_id,
                                                       WaiterConfig=dict(Delay=waiter_delay,
                                                                         MaxAttempts=waiter_max_attempts))
        stack.reload()
        output = dict(map(lambda o: (o.get('OutputKey'), o.get('OutputValue')), stack.outputs))
...
class SAM(CloudFormationService):
    ...
    def build(self,
        ...
        cmd = ['sam', 'build', '--base-dir', os.fspath(_PROJECT_ROOT), '--template', template_fspath]
        if parameters:
            cmd.extend(['--parameter-overrides',
                        ' '.join(f'ParameterKey={p.name},ParameterValue={p.value}'
        ...
        _log.info('building SAM application:\n  %s', ' '.join(f"'{tok}'" for tok in cmd))
    ...
    def package(self,
        ...
        source_template_name = pathlib.Path(_to_template_fspath(template_name, template_path)).name
        output_template_path = packaged_template_path(source_template_name)
        packaged_template_fspath = os.fspath(output_template_path)
        cmd = ['sam', 'package', '--s3-bucket', self.package_bucket, '--output-template-file', packaged_template_fspath]
        if debug:
            cmd.append('--debug')
        profile = self.profile_name
        if profile:
            cmd.extend(['--profile', profile])
        cmd.extend(['--region', self.region_name])
        _log.info('packaging SAM application:\n  %s', ' '.join(f"'{tok}'" for tok in cmd))
        subprocess.run(cmd, check=True)
    ...
    def create_stack(self,
                     ...
                     use_container: bool = False) -> t.Mapping[str, t.Any]:
        debug = _log.isEnabledFor(logging.DEBUG)
        self.build(template_name, template_path, parameters=parameters, use_container=use_container, debug=debug)
        packaged_template_path = self.package(template_name, template_path, debug=debug)
        return super().create_stack(stack_name,
                                    template_name=None,
