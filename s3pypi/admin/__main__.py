...
_user_entry_re: t.ClassVar[t.Pattern] = re.compile(
...
def s3_text_object_lines(
    s3_object, encoding="utf-8", raise_not_found=True
) -> t.Iterator[str]:
    _log.debug(
        "fetching the content of s3://%s/%s", s3_object.bucket_name, s3_object.key
    )
    try:
        ...
        if response_meta.get("HTTPStatusCode") != 404 or raise_not_found:
            raise
        _log.info(
            "missing s3 object s3://%s/%s treated as empty",
    ...
    if body_stream is None:
        _log.warning(
            "received empty object body for s3://%s/%s",
            s3_object.bucket_name,
            s3_object.key,
        )
        return
    ...
    try:
        ...
        _log.debug(
            "closing the content of s3://%s/%s", s3_object.bucket_name, s3_object.key
        )
        body_stream.close()


def update_user_store(
    user_store_object, normalized_username, update_function, raise_object_not_found=True
) -> None:
    ...
    try:
        lines = list(
            s3_text_object_lines(
                user_store_object, raise_not_found=raise_object_not_found
            )
        )
        user_index = next(
            (i for i, l in enumerate(lines) if user_entry_predicate(l)), -1
        )
        ...
        if response_meta.get("HTTPStatusCode") != 404:
            raise
        _log.warning(
            "s3 object s3://%s/%s not found",
            user_store_object.bucket_name,
            user_store_object.key,
        )
        exit(3)
...
def set_password(args: argparse.Namespace) -> None:
    ...
    def add_or_replace_user_entry(lines: t.List[str], idx: int) -> None:
        ...
        if 0 <= idx < len(lines):
            _log.debug(
                "updating user store line %d with entry for user %s",
                idx + 1,
                normalized_username,
            )
            lines[idx] = new_entry
            ...
            _log.debug(
                "adding user store line %d with entry for user %s",
                len(lines),
                normalized_username,
            )
...
def delete_user(args: argparse.Namespace) -> None:
    ...
    def delete_user_entry(lines: t.List[str], idx: int) -> None:
        if 0 <= idx < len(lines):
            _log.debug(
                "deleting user store line %d with entry for user %s",
                idx + 1,
                normalized_username,
            )
            del lines[idx]
...
def parse_args(raw_args: t.Sequence[str]) -> argparse.Namespace:
    ...
    pset = sp.add_parser(
        "set_password",
        help="adds or updates the specified user's entry in the user store",
    )
    pset.add_argument(
        "-c",
        "--create",
        action="store_true",
        help="create S3 object if it does not exist yet",
    )
    pset.add_argument(
        "-s", "--from-stdin", action="store_true", help="read pw from stdin"
    )
    pset.add_argument("username", metavar="USERNAME", help="the name of the user")
    pset.set_defaults(handler=set_password)

    pdel = sp.add_parser(
        "delete", help="delete any entry for the specified user from the user store"
    )
